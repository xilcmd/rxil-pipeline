...
> **History.** Written for the Python dashboard, which served files through
> Gradio's `/gradio_api/file=` endpoint. The Rust `xil gui` serves the workspace
> under `/ws/`; the relative-URL and local-cache findings below still apply.
