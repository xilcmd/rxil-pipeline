...
> Section names keep the `XILP`/`XILU` numbers of the Python modules each
> command was ported from; `xil --help` prints the same tags. Every command
> here runs as `xil <command>`.
...
> SFX/music/ambience generation is unaffected. Needs network access to Google Translate.
...
Every command has a man page, generated from its option definitions and
committed under `man/man1/` in the repository.

```bash
man -l man/man1/xil-parse.1      # read one in place
export MANPATH="$PWD/man:$(manpath 2>/dev/null)"
man xil-produce
man xil                          # dispatcher overview (lists all commands)
```

Regenerate them after changing any command's options; CI fails when the
committed pages are stale:

```bash
xil --generate-man man/man1
```
